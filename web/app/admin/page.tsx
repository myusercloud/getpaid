    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="text-center">
          ...
          <p className="text-2xl font-bold text-slate-900 tracking-tight">{data?.totalUsers ?? 0}</p>
        </Card>
        <Card className="text-center">
          <p className="text-xs text-slate-500 mb-1">Active Members</p>
          <p className="text-2xl font-bold text-sky-600 tracking-tight">{data?.activeMembers ?? 0}</p>
        </Card>
        <Card className="text-center">
          <p className="text-xs text-slate-500 mb-1">Tasks Today</p>
          <p className="text-2xl font-bold text-slate-900 tracking-tight">{data?.tasksCompletedToday ?? 0}</p>
        </Card>
        <Card className="text-center">
          <p className="text-xs text-slate-500 mb-1">Total Virtual KES</p>
          <p className="text-2xl font-bold text-slate-900 tracking-tight">{formatKES(data?.totalVirtualKES ?? 0)}</p>
        </Card>
      </div>

      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          <h2 className="text-sm font-medium text-slate-900 mb-4">Recent transactions</h2>
              ...
              {data.recentTransactions.map((tx) => (
                <li key={tx.id} className="py-2 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{tx.userName} · {formatDate(tx.createdAt)}</p>
                  </div>
        ...
        <Card>
          <h2 className="text-sm font-medium text-slate-900 mb-4">Top earners</h2>
          {data?.topEarners && data.topEarners.length > 0 ? (
            <ul className="divide-y divide-slate-100">
              {data.topEarners.map((u, i) => (
                <li key={u.id} className="py-2 flex items-center gap-3">
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${i === 0 ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{i + 1}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-slate-800 truncate">{u.name}</p>
                    <p className="text-xs text-slate-400">{u.email}</p>
                  </div>
                  <span className="text-sm font-medium text-slate-700">{formatKES(u.totalEarned)}</span>
                </li>
              ))}
            </ul>
