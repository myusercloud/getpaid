    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="text-center">
          ...
          <p className="text-2xl font-bold text-slate-900 tracking-tight">{stats?.totalReferrals ?? 0}</p>
        </Card>
        <Card className="text-center">
          <p className="text-xs text-slate-500 mb-1">Activated</p>
          <p className="text-2xl font-bold text-emerald-600 tracking-tight">{stats?.activeReferrals ?? 0}</p>
        </Card>
        <Card className="text-center">
          <p className="text-xs text-slate-500 mb-1">Earned</p>
          <p className="text-2xl font-bold text-slate-900 tracking-tight">{formatKES(stats?.totalBonus ?? 0)}</p>
        </Card>
      </div>

      <Card>
        <h2 className="text-sm font-medium text-slate-900 mb-4">Your referrals</h2>
        {referrals && referrals.length > 0 ? (
          <ul className="divide-y divide-slate-100">
            {referrals.map((r) => (
              <li key={r.id} className="py-3 flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-800">{r.referred.name}</p>
        ...
        ) : <p className="text-sm text-slate-400 text-center py-6">No referrals yet — share your link to get started</p>}
      </Card>

      <Card>
        <h2 className="text-sm font-medium text-slate-900 mb-4">Top referrers</h2>
